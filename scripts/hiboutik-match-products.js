...
async function fetchAllHiboutikProducts() {
  ...
  while (true) {
    ...
    if (!res.ok) throw new Error(`Hiboutik products page ${page}: HTTP ${res.status}`);
    const items = await res.json();
    all = all.concat(items);
    const link = res.headers.get('link') || '';
    if (link.includes('rel="next"') && !link.includes(`?p=${page}>; rel="next", <`)) {
      // il existe une page suivante distincte de la page courante
      const hasNext = /rel="next"/.test(link) && !link.match(new RegExp(`\\?p=${page}>;\\s*rel="next"`));
      if (!hasNext) break;
      page++;
    } else break;
  }
...
(async () => {
  const [hbProducts, pv, existing] = await Promise.all([
    fetchAllHiboutikProducts(), fetchProduitsVendus(), fetchExistingMapping()
  ]);
  const alreadyConfirmed = new Set(existing.filter(e => e.confirmed).map(e => e.hiboutik_product_id));

  // Index produits_vendus par nom normalisé — détecte les ambiguïtés (2 produits, même nom normalisé)
  const byName = {};
  for (const p of pv) {
  ...
  for (const hp of hbProducts) {
    if (hp.product_arch) continue; // produit archivé côté caisse, ignoré
    if (alreadyConfirmed.has(hp.product_id)) { skipped++; continue; } // déjà confirmé manuellement, on n'y touche pas

    const key = normalize(hp.product_model);
    const candidates = byName[key] || [];
    if (candidates.length === 1) {
      rows.push({ hiboutik_product_id: hp.product_id, hiboutik_nom: hp.product_model, produit_vendu_code: candidates[0], confirmed: true });
      matched++;
    } else {
      rows.push({ hiboutik_product_id: hp.product_id, hiboutik_nom: hp.product_model, produit_vendu_code: null, confirmed: false });
      nonMatches.push(hp.product_model + (candidates.length > 1 ? '  (ambigu: plusieurs produits portent ce nom)' : ''));
    }
  }
